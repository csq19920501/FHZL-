...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.menuAnimateType = FFDropDownMenuViewAnimateType_FallFromTop;
        self.ifShouldScroll = YES;
...
#pragma mark - csq快速实例化一个菜单对象<farst instance>添加判断是否居右

+ (instancetype)ff_DefaultStyleDropDownMenuWithMenuModelsArray:(NSArray *)menuModelsArray menuWidth:(CGFloat)menuWidth eachItemHeight:(CGFloat)eachItemHeight menuRightMargin:(CGFloat)menuRightMargin triangleRightMargin:(CGFloat)triangleRightMargin
                                                    csqIsRight:(BOOL)csqIsRigth
    {
...
- (void)setup {
    ...
    self.triangleView.frame = CGRectMake((horizonWidth + self.triangleSize.width)/2  - self.triangleSize.width, self.realTriangleY, self.triangleSize.width, self.triangleSize.height);
...
//    CGFloat menuViewHeight = self.menuBarHeight >= 0 ? self.menuBarHeight : self.eachMenuItemHeight * self.menuModelsArray.count;
    CGFloat menuViewHeight = self.menuBarHeight >= 0 ? self.menuBarHeight :   self.menuModelsArray.count < 8?self.eachMenuItemHeight * self.menuModelsArray.count:self.eachMenuItemHeight *8;
    self.menuViewFrame = CGRectMake((horizonWidth + self.menuWidth)/2   - self.menuWidth , CGRectGetMaxY(self.triangleView.frame), self.menuWidth, menuViewHeight);
