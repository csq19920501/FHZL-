...
+ (CLLocationCoordinate2D)bd09ToWgs84:(CLLocationCoordinate2D)location
{
 ...
 //百度官方算法（GPS坐标系转百度）将百度坐标系a 转换成b  return a*2-b
+ (CLLocationCoordinate2D)CSQbd09ToWgs84:(CLLocationCoordinate2D)location{
  
    NSDictionary* testdic = BMKConvertBaiduCoorFrom(location,BMK_COORDTYPE_GPS);
    CLLocationCoordinate2D baiduCoor = BMKCoorDictionaryDecode(testdic);//转换后的百度坐标
    CGFloat longStr = location.longitude *2 - baiduCoor.longitude;
    CGFloat latiStr = location.latitude *2 - baiduCoor.latitude;
    CLLocationCoordinate2D gpsCoordinate ;
    gpsCoordinate.latitude = latiStr;
    gpsCoordinate.longitude = longStr;
    return gpsCoordinate;
}
//百度官方算法    注意这里名字写法有问题  返回的是百度坐标
+ (CLLocationCoordinate2D)BD_bd09ToWgs84:(CLLocationCoordinate2D)location{
