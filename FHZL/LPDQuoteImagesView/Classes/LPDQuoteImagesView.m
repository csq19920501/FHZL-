...
- (instancetype)initWithCountPerRowInView:(NSUInteger)ArrangeCount cellMargin:(CGFloat)cellMargin{
...
- (instancetype)initWithFrame:(CGRect)frame withCountPerRowInView:(NSUInteger)ArrangeCount cellMargin:(CGFloat)cellMargin{
     if(self = [super initWithFrame: frame]){
        ...
        self.backgroundColor = RGB(231, 235, 240);
        _selectedPhotos = [[NSMutableArray alloc] init];
        _selectedAssets = [[NSMutableArray alloc] init];
...
- (void)configCollectionView {
  ...
  _collectionView.backgroundColor =  RGB(231, 235, 240);;
...
//  cell.backgroundColor = RGB(231, 235, 240);
    cell.contentView.backgroundColor = RGB(231, 235, 240);
  return cell;
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
  if (indexPath.row == _selectedPhotos.count) {
    [self pushImagePickerController];//takePhoto    pushImagePickerController
  } else { //预览照片或者视频
    ...
    if (isVideo) { // 预览视频
      ...
      [selectImagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
        ...
        [_collectionView reloadData];
          if (_navcDelegate && [_navcDelegate respondsToSelector:@selector(changeConstant)]) {
              [_navcDelegate changeConstant];
          }
...
//系统拍照
-(void)getCameraWithImage:(UIImage *)photeImage{
...
//            }
            ...
            [[LPDImageManager manager] getPhotoWithAsset:assetModel photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            }];
            [self refreshCollectionViewWithAddedAsset:assetModel.asset image:photeImage];
...
- (void)openImagePickerWithSourceType:(UIImagePickerControllerSourceType)sourceType {
    ...
    [self.navcDelegate presentViewController:picker animated:YES completion:nil];
}
