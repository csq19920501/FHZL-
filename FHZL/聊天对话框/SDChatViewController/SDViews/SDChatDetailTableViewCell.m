    ...
    }else //其他
    {
        ...
        if (!chat.patient) {
            ...
            if ([[UserManager sharedInstance].user.iconId integerValue] >= 1 && [[UserManager sharedInstance].user.iconId integerValue] <= 12) {
                ...
                [self.iconHeadBtn setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"ui2_user_icon%@",[UserManager sharedInstance].user.iconId]] forState:UIControlStateNormal];
            }else if([[UserManager sharedInstance].user.iconId integerValue] == 13 ){
                [self.iconHeadBtn sd_setBackgroundImageWithURL:[NSURL URLWithString:USERMANAGER.user.headPicPath] forState:UIControlStateNormal];
            }
            else if( [[UserManager sharedInstance].user.iconId integerValue] == 14){
                [self.iconHeadBtn sd_setBackgroundImageWithURL:[NSURL URLWithString:USERMANAGER.user.upPicPath] forState:UIControlStateNormal];
            }else
            {
                [self.iconHeadBtn setBackgroundImage:[UIImage imageNamed:@"微众世界_头像0"] forState:UIControlStateNormal];
            }
...
-(void)layoutSubviews{
    ...
    self.iconHeadBtn.layer.cornerRadius = self.iconHeadBtn.frame.size.width/2;
    self.iconHeadBtn.layer.masksToBounds = YES;
}
@end
